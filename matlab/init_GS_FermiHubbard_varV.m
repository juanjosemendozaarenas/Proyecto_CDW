...
L = 64; % System size
J = 1; % Hopping
U = -2; % Nearest-neighbor interaction
V_array = 0.55:0.05:1; % On-site interaction

J_coupling = -J*diag(eye(L-1))'; % Array of J
U_coupling = U*diag(eye(L))'; % Array of U
...
for count_file = 1:size(V_array,2)
    ...
    savefile = ['GS_FH1c_L' num2str(L) '_J' num2str(J) '_U' num2str(U) '_V' num2str(V,'%.2f') '_chi' num2str(chi_max) '.mat'];
   
    % Saving current information
    fname = ['./initfiles/initial_GS_FH1c' num2str(count_file+add_to_file) '.mat'];
    save(fname);
