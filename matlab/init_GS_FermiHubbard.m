...
L = 64; % System size
J = 1; % Hopping
U_array = 1; % List of values of on-site interaction
V = -1.38; % Nearest-neighbor interaction
...
chi = 400; % Initial value of chi
chi_max = 1600; % Maximal and chi value of the truncation parameter
delta_chi = 200; % Increase of chi each time maximal error is achieved. Set to zero to keep chi constant.
...
for count_file = 1:size(U_array,2)
    ...
    ExOp.os_operators = tntMatCreateOpArray({sz{1},n{2},dbl{1}}); % Single-site operators
    ExOp.os_labels = {'sz','nup','dbl'};
...
%     ExOp.ap_labels = {'szsz_all','nn_all','pairing_all'};
    ...
    fname = ['./initfiles/initial_GS_FH' num2str(count_file+add_to_file) '.mat'];
    save(fname);
