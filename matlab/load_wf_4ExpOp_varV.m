...
L = 64; % System size
J = 1; % Hopping
U = 1; % Nearest-neighbor interaction
V = -1.35; % On-site interaction
chi_max = 1500; % Maximal and chi value of the truncation parameter
add_to_file = 25;

for cont=1:length(V)
    lname = ['/output/GS2a/GS_FH2a_L' num2str(L) '_J'...
        num2str(J) '_U' num2str(U) '_V' num2str(V(cont),'%.2f')...
    ...
    % Name of file where Expected values will be saved
    savefile = ['GS_FH2a_L' num2str(L) '_J' num2str(J) '_U' ...
        num2str(U) '_V' num2str(V(cont),'%.2f')...
        '_chi' num2str(chi_max) '.mat'];

    % Saving current information
    fname = ['./initfiles/existing_GS_FH2a_4ExpOps'...
