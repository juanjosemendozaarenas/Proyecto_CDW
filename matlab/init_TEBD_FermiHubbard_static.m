...
L = 16; % Total number of sites
J = 1; % Hopping
U_array = [0.5]; % List of values of on-site interaction
...
GS = 0; % 1 to read ground state from DMRG result, 0 to use simple product initial state
if(GS == 0)
    STATE = 'AFM'; % AFM for antiferro, CDW for charge density wave
elseif(GS == 1)
    V_gs = -1; % Value of V of the ground state taken as initial state
...
for count_file = 1:size(U_array,2)
    ...
    nnlt = tntMatCreateOpArray({cdP{1},Pc{1},cdP{2},Pc{2}});
    nnrt = tntMatCreateOpArray({c{1},cd{1},c{2},cd{2}});
    nnparamt = [J_coupling;
                J_coupling;
                J_coupling;
                J_coupling];
    ...
    if(GS == 1) % If starting from a ground state
        gsfile = ['GS_FH_L' num2str(L) '_J' num2str(J) '_U' num2str(U) '_V' num2str(V_gs) '_chi' num2str(chi_max) '.mat']; % Name of file where initial state has been saved
    end
    
    % Name of file where results of time evolution will be saved. For now
    % V = 0 for evolution, so it won't be included in the output file name
    savefile = ['TEBD_FH_' STATE '_L' num2str(L) '_J' num2str(J) '_U' num2str(U) '_chi' num2str(chi_max) '_dt' num2str(dt) '_era' num2str(era) '.mat'];
    
    % Name of files where state of previous era was saved, and where state of current era will be saved
    prev_era_state = ['State_TEBD_FH_' STATE '_L' num2str(L) '_J' num2str(J) '_U' num2str(U) '_chi' num2str(chi_max) '_dt' num2str(dt) '_era' num2str(prev_era) '.mat'];
    curr_era_state = ['State_TEBD_FH_' STATE '_L' num2str(L) '_J' num2str(J) '_U' num2str(U) '_chi' num2str(chi_max) '_dt' num2str(dt) '_era' num2str(era) '.mat'];
